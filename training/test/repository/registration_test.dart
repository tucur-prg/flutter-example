...
main() {
  ...
  group('登録', () {
    test('成功', () async {
      ...
      expect(await obj.accounts('bob', 20), {"id": 1});
    });
    ...
    test('重複', () async {
      when(client.post(any,
              headers: anyNamed('headers'), body: anyNamed('body')))
          .thenAnswer((_) async =>
              Response('{"error": "Duplicates not allowed"}', 200));

      var obj = Registration(client);

      expect(
          () async => await obj.accounts('bob', 20),
          throwsA(isA<RegistrationException>()
              .having((e) => e.message, 'エラーメッセージ', 'Duplicates not allowed')
              .having((e) => e.code, 'エラーコード', -1)));
    });
  });
}
